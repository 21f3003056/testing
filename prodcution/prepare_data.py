...
def create_connection():
    ...
    try:
        ...
        logger.info(f"Connected to SQLite database at {DATABASE}")
        return conn
        ...
        if len(para) > chunk_size:
            ...
            for sentence in sentences:
                ...
                # If adding this sentence would exceed chunk size, save current and start new
                    ...
                    len(sentence_chunk) + len(sentence) > chunk_size and sentence_chunk
                ):
                    chunks.append(sentence_chunk.strip())
...
def process_discourse_files(conn):
    ...
    if count > 0:
        logger.info(
            f"Found {count} existing discourse chunks in database, skipping processing"
        )
        return
...
def process_markdown_files(conn):
    ...
    if count > 0:
        logger.info(
            f"Found {count} existing markdown chunks in database, skipping processing"
        )
        return
...
async def create_embeddings(api_key):
    ...
    async def handle_long_text(
        session, text, record_id, is_discourse=True, max_retries=3
    ):
        ...
        logger.info(
            f"Text exceeds embedding limit for {record_id}: {len(text)} chars. Creating multiple embeddings."
        )
        ...
        for i, subchunk in enumerate(subchunks):
            ...
            if not success:
                logger.error(
                    f"Failed to embed subchunk {i + 1}/{len(subchunks)} for {record_id}"
                )
    ...
    async def embed_text(
        session, text, record_id, is_discourse=True, max_retries=3, part_id=None
    ):
        retries = 0
            ...
            try:
                ...
                async with session.post(url, headers=headers, json=payload) as response:
                    if response.status == 200:
                        ...
                        logger.warning(
                            f"Rate limit reached, retrying after delay (retry {retries + 1}): {error_text}"
                        )
                        ...
                        error_text = await response.text()
                        logger.error(
                            f"Error embedding text (status {response.status}): {error_text}"
                        )
                        return False
            except Exception as e:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i in range(0, len(discourse_chunks), batch_size):
            ...
            tasks = [
                handle_long_text(session, text, record_id, True)
                for record_id, text in batch
            ]
            results = await asyncio.gather(*tasks)
            logger.info(
                f"Embedded discourse batch {i // batch_size + 1}/{(len(discourse_chunks) + batch_size - 1) // batch_size}: {sum(results)}/{len(batch)} successful"
            )
        ...
        for i in range(0, len(markdown_chunks), batch_size):
            ...
            tasks = [
                handle_long_text(session, text, record_id, False)
                for record_id, text in batch
            ]
            results = await asyncio.gather(*tasks)
            logger.info(
                f"Embedded markdown batch {i // batch_size + 1}/{(len(markdown_chunks) + batch_size - 1) // batch_size}: {sum(results)}/{len(batch)} successful"
            )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Preprocess Discourse posts and markdown files for RAG system"
    )
    parser.add_argument(
        "--api-key",
        help="API key for aipipe proxy (if not provided, will use API_KEY environment variable)",
    )
    parser.add_argument(
        "--chunk-size",
        type=int,
    ...
    if not api_key:
        logger.error(
            "API key not provided. Please provide it via --api-key argument or API_KEY environment variable."
        )
        return
